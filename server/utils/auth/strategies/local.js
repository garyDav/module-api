  ...
  new Strategy(
    ...
    async function (email, password, done) {
      try {
        ...
        if (!app_usuario) {
          // Si no existe el usuario en la app, buscamos en los usuarios de la web
          if (!web_usuario) {
            return done(boom.unauthorized(), false)
          } else {
            // Si existe el usuario de la web verificamos su contraseña
            if (
              !(await bcrypt.compare(password, web_usuario.dataValues.password))
            ) {
              return done(boom.unauthorized(), false)
            }
